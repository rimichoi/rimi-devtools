 ...
 *
 * 범위를 `(공백 ~ ~)` 로 적으면 물결표가 "부터~까지" 의 구분자와 경계 문자 자신을
 * 동시에 맡아 읽을 수 없다. 구분자를 말로 풀고 문자는 한 번만 보여준다.
 */
export const NON_ASCII_PASSWORD_ERROR =
  '마스터 비밀번호에 출력 가능 ASCII(공백부터 물결표 ~ 까지) 밖의 문자가 있습니다. Java 의 PBEWithMD5AndDES 가 이 비밀번호를 거부하므로, 여기서 만든 값도 Java 에서 풀 수 없습니다.';
